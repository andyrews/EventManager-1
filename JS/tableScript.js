import { pushToList, getList, updateData, deleteData } from '/JS/fireScript.js'
let currentGroup;
...
function createTaskTemp(body, textInput, uniqueKey, status) {
    ...
    editBtn.addEventListener("click", function () {
        if (isEditing == false) {
            ...
            isEditing = false;
            const update = { task: taskTitle.textContent }
            updateData(update, `groups/TASK_CONTAINER/${uniqueKey}`)
        }
    })
    ...
    deleteBtn.addEventListener("click", function () {
        row.remove();
        deleteData(`groups/TASK_CONTAINER/${uniqueKey}`)
    })
